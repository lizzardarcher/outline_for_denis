...
echo "Script completed successfully."
    ...
    logger.info(f'Найдено серверов для активации {servers.count()}')
    if servers:
        for server in servers:
            try:
                logger.info(f'Устанавливам необходимо ПО Marzban-node на сервер {server.ip_address}')
                ...
                ssh_client.close()
                logger.info('Marzban-node успешно установлен')

                try:
                    logger.info('Добавляем ноду на сервер Marzban')
                    marzban = MarzbanAPI()
                    ...
                    if 'True' not in str(new_node):
                        logger.error(f'Ошибка в добавлении ноды\n{new_node}')
                    else:
