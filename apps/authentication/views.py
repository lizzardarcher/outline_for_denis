...
def password_reset_support(request):
    """Восстановление пароля через техподдержку в Telegram (без email)."""
    support_account = getattr(settings, 'SUPPORT_ACCOUNT', 'Domvpnsupport').lower()
    return render(
        request,
        'account/password_reset.html',
        {
            'support_telegram_url': f'https://t.me/{support_account}',
            'support_telegram_username': f'@{support_account}',
        },
    )
